...
import { FaEnvelope, FaLock } from 'react-icons/fa'; // FaUser was not used, so removed it.
...
export default function Configuracoes() {
  ...
  const handleUpdate = async (fieldToUpdate, value) => {
    ...
    if (fieldToUpdate === 'nome') {
      if (!value || value === currentUserData.nome) {
        setError('Nome inválido ou igual ao atual.');
        return;
      }
      updatePayload.nome = value;
    } else if (fieldToUpdate === 'email') {
      if (!value || value === currentUserData.email) {
        setError('Email inválido ou igual ao atual.');
        return;
      }
  ...
  // The condition `if (error && !message)` was causing the component to render an incomplete UI
  // when there was an error. The error and message handling should be within the main return statement
  // so that the entire UI (buttons, forms) is always available, and only the error/message text changes.
